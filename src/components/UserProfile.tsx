...
interface UserProfileProps {
  username: string;
  displayName: string;
  onUpdate: (username: string, displayName: string) => Promise<void>;
}

export function UserProfile({ username, displayName, onUpdate }: UserProfileProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (newUsername !== username) {
        const res = await fetch(`/api/user?username=${encodeURIComponent(newUsername)}`);
      ...
      await onUpdate(newUsername.trim(), newDisplayName.trim());
      setOpen(false);
