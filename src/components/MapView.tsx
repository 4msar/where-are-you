  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className={`text-sm font-bold ${isCurrentUser ? "text-white" : "text-gray-700"}`}>
      ...
      {infoOpen && marker && (
        <InfoWindow anchor={marker} onCloseClick={() => setInfoOpen(false)}>
          <div className="p-1 min-w-[120px]">
            ...
            {isCurrentUser && (
              <p className="text-xs text-blue-600 font-medium mt-1">📍 You are here</p>
            )}
            <p className="text-xs text-gray-400 mt-1">
              Updated:{" "}
              {new Date(user.lastUpdated).toLocaleString(undefined, {
                dateStyle: "short",
                timeStyle: "short",
              })}
            </p>
...
export function MapView({ apiKey, currentUser, allUsers }: MapViewProps) {
  const defaultCenter = currentUser?.location ?? { lat: 20, lng: 0 };
  const defaultZoom = currentUser ? 13 : 2;
