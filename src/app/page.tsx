...
const USER_STORAGE_KEY = "where-are-you:user";
...
export default function HomePage() {
  ...
  // Initialize user from localStorage or create a new one
  useEffect(() => {
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          intervalRef.current = setInterval(() => {
            navigator.geolocation.getCurrentPosition(
              (pos) => {
                saveLocation(pos.coords.latitude, pos.coords.longitude, userData);
              },
              () => {},
  ...
  const handleProfileUpdate = useCallback(
    async (newUsername: string, newDisplayName: string) => {
      if (!user) return;
    ...
    [user, currentLocation, saveLocation]
  );
    ...
    <div className="relative w-full h-screen overflow-hidden bg-gray-100">
      ...
      <div className="absolute top-4 left-4 right-4 flex items-start justify-between gap-3 pointer-events-none">
        ...
        <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-4 py-2 shadow-md border border-gray-100 pointer-events-auto">
          ...
          <span className="font-bold text-gray-800 text-sm">Where Are You</span>
        </div>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 pointer-events-none">
        <div className="flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-4 py-2 shadow-md border border-gray-100 text-sm">
          ...
          {locationStatus === "granted" && !isUpdating && currentLocation && (
            <>
          ...
          {locationStatus === "denied" && (
            <>
              ...
              <span className="text-gray-600">Location access denied</span>
              <button
          ...
          {locationStatus === "error" && (
            <>
              ...
              <span className="text-gray-600">Could not get location</span>
              <button
                className="pointer-events-auto ml-1 text-blue-600 font-medium hover:underline"
                onClick={() => {
                  setLocationStatus("idle");
